...
# та же нейронка о выборе девочкой мальчика пройдет обучение

# такая будет выбрана функция активация (гиперболический тангенс), она дифференцируема во всем диапазоне
def f(x):
    return 2 / (1 + np.exp(-x)) - 1


# это соответственно производная
def df(x):
    return 0.5 * (1 + x) * (1 - x)
...
def train(epoch):
    ...
    for k in range(N):
        x = epoch[np.random.randint(0, count)] # случайный выбор входного сигнала из выборки
